...
from src.live_features import add_universal_live_features
...
def process_match(g, mid, year, slam):
    ...
    # Prior-stat helpers use only points before the current point to avoid leakage.
    ...
    # Leakage-safe cumulative and recent live features shared by every model.
    p1_points_won = prior_sum(pw == 1)
    p2_points_won = prior_sum(pw == 2)
    ...
    p1_break_points_won = prior_sum(p1_break_point_event & (pw == 1))
    p2_break_points_won = prior_sum(p2_break_point_event & (pw == 2))

    p1_first_serve_won_event = (server == 1) & (serve_number == 1) & (pw == 1)
    p2_first_serve_won_event = (server == 2) & (serve_number == 1) & (pw == 2)
    p1_first_serve_points_won = prior_sum(p1_first_serve_won_event)
    p2_first_serve_points_won = prior_sum(p2_first_serve_won_event)
    ...
    p1_momentum = rolling_prior_rate(all_points, pw == 1, window=12)
    p2_momentum = rolling_prior_rate(all_points, pw == 2, window=12)
...
def main():
    ...
    # Store one universal definition of model-independent live features.
