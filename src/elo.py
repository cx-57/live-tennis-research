...
# Add the project root so src imports work when this file is run directly
...
# Tournament and round mappings used to align tour results with Slam point data
SLAM_NAME = {
...
def norm(s):
    """Normalize player and tournament names for matching across datasets"""
...
def last_name(s):
    """Extract the last name from a normalized player name"""
    parts = s.split()
    return parts[-1] if parts else s


def load_tour():
    """Load ATP and WTA tour match results from 2005 through 2024"""
    frames = []

    # Read yearly tour files and keep only columns needed for Elo
    ...
    # Clean missing matches and sort them in match-play order
...
def run_elo(tour):
    """Run an Elo update through the tour match history"""
    ...
    # Store pre-match ratings before updating Elo after each match
...
def main():
    # Build chronological tour Elo ratings
    tour = load_tour()
    tour, n_players = run_elo(tour)

    print(f"tour matches 2005-2024: {len(tour)}  players rated: {n_players}")

    # Normalize and filter tour results down to Grand Slam matches
    ...
    # Build full-name and last-name lookup tables for assigning Elo to Slam matches
    ...
    # Remove ambiguous last-name matches so incorrect Elo is not assigned
    for key in last_clash:
        by_last.pop(key, None)

    names = {}

    # Read player names from Slam metadata files
    ...
    # Load one metadata row per processed point-by-point match
    meta = (
        pd.read_parquet(POINTS, columns=["match_id", "year", "slam"])
        .groupby("match_id")
        .agg(year=("year", "first"), slam=("slam", "first"))
        .reset_index()
    )
    ...
    # Match each point-by-point Slam match to its pre-match Elo values
    ...
    # Save final Elo features for model training and evaluation
