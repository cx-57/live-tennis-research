...
def process_match(g, mid, year, slam):
    ...
    # All running features exclude the current point to prevent target leakage.
    ...
    def run_rate(is_sample, is_success):
        ...
        return np.where(n > 0, wins / np.maximum(n, 1), 0.5), n
    ...
    def rolling_prior_rate(is_sample, is_success, window, default=0.5):
        ...
        return (wins / n.replace(0, np.nan)).fillna(default).to_numpy()
    ...
    p1_recent_ace_rate = rolling_prior_rate(
        server == 1,
        p1_ace == 1,
        window=24,
    )
    p2_recent_ace_rate = rolling_prior_rate(
        server == 2,
        p2_ace == 1,
        window=24,
    )
    ...
    p1_first_serve_won_event = (
        (server == 1) & (serve_number == 1) & (point_winner == 1)
    )
    p2_first_serve_won_event = (
...
def main():
    ...
    for point_file in sorted(glob.glob(f"{SLAM_DIR}/*-points.csv")):
        base = os.path.basename(point_file).replace("-points.csv", "")
