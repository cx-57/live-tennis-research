...
Ratings are updated match by match in chronological order with a 538-style decaying step size,
so a player's rating going into any match reflects only earlier results. Building the rating
from the whole tour rather than from slam results alone removes the cold start where a player
would otherwise enter their first slam at the default 1500. The ratings are then attached to the
slam point-by-point matches by season, event and player names, oriented to player 1 and player 2.
"""
...
from common import ATP_DIR, WTA_DIR, SLAM_DIR, POINTS, ELO
...
def main():
    ...
    meta = pd.read_parquet(POINTS, columns=["match_id", "year", "slam"]).groupby(
        "match_id"
    ).agg(year=("year", "first"), slam=("slam", "first")).reset_index()
    ...
    for r in meta.itertuples():
        ...
        p1_is_winner = (n1 == winner_key) if by_full_match else (
            last_name(n1) == winner_key
        )
