...
Win probability is always reported from player 1's perspective. The symmetric case pa == pb
recovers the equal-players anchor, where the win probability is a pure function of the score.
"""
...
def make_solver(pa, pb):
    ...
    @lru_cache(None)
    def game(s, i, j):
        # P(server wins a game) | server wins each point with prob s, current points i:j
        if i >= 4 and i - j >= 2:
            return 1.0
        if j >= 4 and j - i >= 2:
            return 0.0
    ...
    def tb_server(k):
        # tiebreak: player 1 serves point 0, then the serve alternates in pairs
        return 1 if (((k + 1) // 2) % 2 == 0) else 2

    @lru_cache(None)
    def tb(a, b):
        # P(player 1 wins a tiebreak) | player 1 served the first tiebreak point
        if a >= 7 and a - b >= 2:
            return 1.0
        if b >= 7 and b - a >= 2:
            return 0.0
        ...
        if a >= 6 and b >= 6 and a == b:
            # win by two from level: closed form over the next two points
    ...
    def setwp(g1, g2, p1serve):
        ...
        if g1 >= 7 and g2 >= 7 and g1 == g2:
            # advantage set with no final tiebreak: closed form over the next two games
            wA, wB = holdA, 1 - holdB
            return wA * wB / (wA * wB + (1 - wA) * (1 - wB))

        p1_game = holdA if p1serve else (1 - holdB)
        return p1_game * setwp(g1 + 1, g2, not p1serve) + (1 - p1_game) * setwp(g1, g2 + 1, not p1serve)

    # win probability of a fresh set, averaged over who serves first (server unknown)
    ...
    def matchwp(s1, s2, best_of, setp):
        ...
        if s2 >= need:
            return 0.0
        return setp * matchwp(s1 + 1, s2, best_of, set0) + (1 - setp) * matchwp(s1, s2 + 1, best_of, set0)
    ...
    def wp(best_of, s1, s2, g1, g2, p1serve, p1pts, p2pts, tie):
        if tie:
            ...
            setp = p1_game * setwp(g1 + 1, g2, not p1serve) + (1 - p1_game) * setwp(g1, g2 + 1, not p1serve)
...
def predict(df, pa, pb, state_cols):
    """Win probability for every row in df. pa, pb are per-row serve probabilities (arrays)
    or scalars. Results are memoised over the rounded serve probabilities and the score state,
    so the few thousand distinct states are each solved once."""
    ...
    for i in range(len(df)):
        ...
        key = (
            pa[i], pb[i], int(bo), int(s1), int(s2),
            int(g1), int(g2), bool(p1s), int(q1), int(q2), bool(tie)
        )
        ...
        if v is None:
            try:
                v = solver(pa[i], pb[i])(
                    int(bo), int(s1), int(s2), int(g1), int(g2),
                    bool(p1s), int(q1), int(q2), bool(tie)
                )
