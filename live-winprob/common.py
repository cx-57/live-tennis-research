...
STATE = ["best_of", "p1_sets", "p2_sets", "p1_games", "p2_games",
         "p1_serving", "p1_score", "p2_score", "tiebreak"]


def load(with_elo=True):
...
def report(name, y, p):
    p = np.clip(p, 1e-6, 1 - 1e-6)
    print(f"{name:34s} logloss={log_loss(y, p, labels=[0, 1]):.4f}  "
          f"brier={brier_score_loss(y, p):.4f}  accuracy={accuracy_score(y, p > 0.5):.4f}")
